...
@section('title', 'Admin - Nouvelle série')

@section('content')
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-3xl font-bold">Nouvelle série</h1>
        <a href="{{ route('admin.series') }}" class="text-gray-300 hover:text-white">← Retour</a>
    </div>

    <form method="POST" action="{{ route('admin.series.store') }}" enctype="multipart/form-data" class="bg-gray-800 rounded-lg p-6 space-y-4" data-ts-validate="form" novalidate>
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input id="series_slug_fr" type="text" value="" readonly class="w-full px-4 py-3 bg-gray-900/60 border border-gray-700 rounded-lg text-gray-200" />
            ...
            <div>
                ...
                <input id="series_slug_en" type="text" value="" readonly class="w-full px-4 py-3 bg-gray-900/60 border border-gray-700 rounded-lg text-gray-200" />
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input id="series_release_year" name="release_year" value="{{ old('release_year') }}" type="number" min="1900" max="{{ date('Y') }}" class="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg" />
            </div>
        </div>
        ...
        <div class="bg-gray-900/30 border border-gray-700/60 rounded-lg p-4">
            ...
            @php
                $selectedLangs = old('video_languages', ['fr']);
            ...
            <div>
                ...
                <div id="poster_preview_wrap" class="mt-3 flex items-center gap-3">
                    <div class="relative h-20 w-14 rounded-lg overflow-hidden border border-white/10 bg-black/20">
                        <div class="absolute inset-0 skeleton hidden"></div>
                        <img id="poster_preview_img" alt="Poster" class="absolute inset-0 w-full h-full object-cover opacity-0 transition-opacity duration-200 hidden"
                             onload="this.classList.remove('opacity-0'); this.parentElement?.querySelector('.skeleton')?.classList.add('hidden')"
                             onerror="this.classList.add('hidden'); this.parentElement?.querySelector('.skeleton')?.classList.add('hidden')" />
                    </div>
                    <div class="min-w-0">
                        <div class="text-xs text-gray-400">Aperçu</div>
                        <a id="poster_preview_link" href="#" target="_blank" rel="noopener" class="text-xs text-gray-200 hover:text-white underline break-all hidden">Ouvrir</a>
                    </div>
                </div>
                <div class="mt-2">
            ...
            <div>
                ...
                <div id="cover_preview_wrap" class="mt-3 flex items-center gap-3">
                    <div class="relative h-20 w-36 rounded-lg overflow-hidden border border-white/10 bg-black/20">
                        <div class="absolute inset-0 skeleton hidden"></div>
                        <img id="cover_preview_img" alt="Cover" class="absolute inset-0 w-full h-full object-cover opacity-0 transition-opacity duration-200 hidden"
                             onload="this.classList.remove('opacity-0'); this.parentElement?.querySelector('.skeleton')?.classList.add('hidden')"
                             onerror="this.classList.add('hidden'); this.parentElement?.querySelector('.skeleton')?.classList.add('hidden')" />
                    </div>
                    <div class="min-w-0">
                        <div class="text-xs text-gray-400">Aperçu</div>
                        <a id="cover_preview_link" href="#" target="_blank" rel="noopener" class="text-xs text-gray-200 hover:text-white underline break-all hidden">Ouvrir</a>
                    </div>
                </div>
                <div class="mt-2">
                    <div class="w-full h-2 bg-black/30 rounded overflow-hidden">
        ...
        <div class="bg-gray-900/40 border border-red-500/30 rounded-lg p-4">
            ...
            <p class="text-xs text-gray-400 mb-3">Cochez <strong>En vedette</strong> pour le carrousel. Plus l’ordre est petit, plus la série est en haut.</p>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm mb-2">Ordre d’affichage (carrousel)</label>
                    <input name="sort_order" type="number" min="-9999" max="99999" value="{{ old('sort_order', 0) }}" class="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg" />
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <label class="flex items-center gap-2">
                ...
                <input type="checkbox" name="is_exclusive" value="1" class="rounded" />
                <span>Exclusif</span>
            </label>
            <label class="flex items-center gap-2">
            ...
            <label class="flex items-center gap-2">
                ...
                <input type="checkbox" name="is_trending" value="1" class="rounded" />
                <span>Tendance</span>
            </label>
            <label class="flex items-center gap-2">
        ...
        <div class="bg-gray-900/30 border border-gray-700/60 rounded-lg p-4">
            ...
            <div id="published_at_wrap" class="mt-4 {{ $pubMode === 'scheduled' ? '' : 'hidden' }}">
                ...
                />
                <p class="text-xs text-gray-400 mt-2">
                    Si programmé, la page affiche “Disponible le …” + bouton “Notifier moi”.
                </p>
            </div>
        </div>
        ...
        <div class="bg-gray-900/30 border border-gray-700/60 rounded-lg p-4">
            <div class="flex items-center justify-between gap-3 flex-wrap mb-3">
                <div>
                    ...
                    <div class="text-xs text-gray-400">Ces genres alimentent la page “Classification”.</div>
                </div>
        ...
        <div class="pt-2">
            <button id="series_submit" class="px-6 py-3 bg-red-600 hover:bg-red-700 rounded-lg font-semibold transition">
                Créer
            </button>
...
<script>
(() => {
  const form = document.querySelector('form[action="{{ route('admin.series.store') }}"]');
  ...
  async function checkSlug(slug) {
    ...
    const res = await fetch(`${checkUrl}?slug=${encodeURIComponent(slug)}`, { headers: { 'Accept': 'application/json' } });
  ...
  function renderSlugState() {
    ...
    // Désactiver le bouton tant que le slug est en doublon
    disabledBySlug = slugExists;
    applyDisabledState();
  }
  ...
  form?.addEventListener('invalid', () => {
    if (!btn) return;
    // On ne réactive pas si c'est un doublon slug.
    if (!disabledBySlug) {
      btn.disabled = false;
  ...
  // (Si la validation côté front empêche l'envoi, on ne bloque pas le bouton.)
  form?.addEventListener('submit', (e) => {
    window.setTimeout(() => {
      ...
      // Ici: vraie soumission => lock anti-double click
      btn.disabled = true;
      ...
      if (xhr.status >= 200 && xhr.status < 300 && data?.success) {
        ...
        if (type === 'poster') {
          ...
          if (img) {
            ...
            img.src = data.url;
            img.dataset.fallback = String(data.url);
            window.talashowRevealImageAfterSrcChange?.(img);
            img.onerror = function(){
              const fb = this.dataset?.fallback;
              if (fb && this.src !== fb) { this.src = fb; return; }
              this.classList.add('hidden');
              this.parentElement?.querySelector('.skeleton')?.classList.add('hidden');
            };
          }
          if (link) { link.classList.remove('hidden'); link.href = data.url; link.textContent = data.url; }
        }
        if (type === 'cover') {
